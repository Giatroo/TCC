...
ofender.~\citep{dicio_sarc, dicio_irony}
...
se fala e elementos não verbais.~\citep{wallace-etal:2014:ironic-context}
...
divididos em duas ou três partes que são chamadas de \textit{dados de
...
\cite{barbieri:2014:modelling-sarcasm}, por sua vez, criam um dos conjuntos mais
...
de incongruência presente nele e criam características que chamam de
...
Dentro do campo do algoritmos de aprendizado supervisionado estão os algoritmos
...
\textit{random forests} (RF), \textit{support vector machines} (SVM),
...
Além dos modelos de aprendizado tidos como mais ``clássicos'', modelos de
...
sentidos parecidos são representadas por vetores parecidos.
...
\cite{hazarika-etal:2018:cascade} criam um método para representar
...
alguns dos métodos de regras vistos na seção
...
processamento sequencial anteriormente mencionado, os \textit{transformers}
criam um novo problema que é o de não representar adequadamente a característica
sequencial de suas entradas. Como não se utiliza RNNs, pode-se pensar em um
...
A principal inovação desse modelo é que ele possui seu treinamento através de
uma metodologia denominada \textit{masked language model} (MLM) (do inglês,
modelo de linguagem mascarada. Nesse tipo de treinamento, alguns dos
\textit{tokens} da sequência de entrada são aleatoriamente mascarados (isto é,
trocados por um \textit{token} específico \texttt{<MASK>}) e a tarefa alvo do
treinamento é justamente identificar quais eram as palavras originais, antes de
serem mascaradas.
...
qual palavra ela era originalmente. Além disso, esse tipo de treinamento é não
supervisionado, o que significa que não preciso de conjuntos de dados
manualmente anotados, possibilitando usar todo tipo de texto disponível na
\textit{internet}.
